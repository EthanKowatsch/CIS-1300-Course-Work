 ...
 * 
 * Return Value(s):
 *  Returns the Guanine (G) and Cytosine (C) nucleotides in the form of a percentage of the DNA sequence.
 * 
 * Side Effects:
 *  None.
 */
...
float calculate_gc_content(const char *dna_sequence) {
    ...
    // Declare variable to store the total Guanine (G) and Cytosine (C) nucleotides count
    ...
    // Declare variables to store the total counts of Guanine (G) and Cytosine (C) nucleotides
    int G_count = 0, C_count = 0;

    // Iterate through the DNA sequence string until the end counting all the Guanine (G) and Cytosine (C) nucleotides
...
int find_motif(const char *dna_sequence, const char *motif) {
    ...
    // Check if the motif has a value, if not, return -1
    ...
    for(int i = 0; i <= last_possible_index; i++) {
        // Declare integer for if the motif is within the sequence (1 meaning true, 0 being false)
 ...
 * 
 * Return Value(s):
 *  Returns the total Hamming distance between the two DNA sequence strings.
 *  Returns -1 (false) if the strings aren't the same size.
 *  Returns 0 (no difference) if the strings are the same.
 * 
 * Side Effects:
 *  None.
 */
...
int are_anagrams(const char *seq1, const char *seq2) {
    ...
    for(int i = 0; i < seq1_length; i++) {
        // Switch statement to determine which nucleotide is at the given index in the first sequence
        switch(seq1[i]) {
            case 'A':
        ...
        // Switch statement to determine which nucleotide is at the given index in the second sequence
        switch(seq2[i]) {
            case 'A':
