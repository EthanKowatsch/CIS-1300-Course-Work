...
void run_launch_sequence(int start_count) {
    ...
    while (start_count >= 0) {
        ...
        // Subtract one away from start_countdown
        start_count--;
    }
}
...
float simulate_asteroid_rendezvous(float *pos_A, float *pos_B, float velocity_A, float velocity_B, float safety_threshold) {
    // Declare float variable to save the distance between asteroids variable
    float distance = 0.0f;
    
    // Calculate initial distance between the variables
    distance = *pos_B - *pos_A;
    
    // Loop to calculate the distance after each movement
    while(distance > safety_threshold) {

        // Declare a float variable to store the next calculated distance
        float next_distance = 0.0f;
        
        // Calculate the next distance using the positions and velocities of the asteroids 
        next_distance = (*pos_B - velocity_B) - (*pos_A + velocity_A);

        // Check if the next distance will be less than the threshold and if it is, exit the loop
        if (next_distance < safety_threshold){
            break;
        } 

        // Update the pointer variables with their respective velocity values
        *pos_A += velocity_A;
        *pos_B -= velocity_B;

        // Calculate new distance of the asteroids
        distance = *pos_B - *pos_A;
    }
...
double survive_asteroid_field(float *core_energy, double *hull_integrity, int mission_cycles, float base_energy_drain, float damage_per_wave, int wave_frequency) {
    ...
    while(current_cycle <= mission_cycles) {
        ...
        *core_energy -= base_energy_drain;
        ...
        // Update current cycle if the ship is still alive
        current_cycle++;
    }
